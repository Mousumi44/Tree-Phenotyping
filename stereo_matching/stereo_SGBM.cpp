...
enum { STEREO_BINARY_BM, STEREO_BINARY_SGBM };
...
int main(int argc, char** argv)
{
    ...
    if (algo==STEREO_BINARY_SGBM){
        cout <<"Algorithm: "<< "Semi Global Block Matching"<<"\n";

    }
    else if (algo==STEREO_BINARY_BM){
        cout <<"Algorithm: "<< "Block Matching"<<"\n";
    ...
    else if (algo == STEREO_BINARY_SGBM)
    {
        // we set the corresponding parameters
                                                   ...
                                                   int &aggregation_window, int &P1, int &P2, float &scale, int &algo, int &binary_descriptor_type, int &success)
{
    ...
    left = parser.get<string>("left");
    right = parser.get<string>("right");
    ...
    algo = parser.get<string>("algorithm") == "sgbm" ? STEREO_BINARY_SGBM : STEREO_BINARY_BM;
